...
describe("build", () => {
  ...
  it("removes obsolete generated runtime handlers from the output directory", async () => {
    const directory = await mkdtemp(join(tmpdir(), "callsite-cli-"));
    const configPath = join(directory, "callsite.config.mjs");
    const outDir = join(directory, "out");

    await mkdir(outDir);
    await writeFile(join(outDir, "handler.ts"), "throw new Error('stale generated artifact');");
    await writeFile(
      configPath,
      `export default {
        version: 1,
        capabilities: [{
          id: "demo.cleanup",
          intent: "Verify obsolete generated artifacts are removed.",
          input: { type: "object" },
          output: { type: "object" },
          destructive: false,
          errors: [],
    ...
    await expect(access(join(outDir, "handler.ts"))).rejects.toMatchObject({
      code: "ENOENT"
    });
    await expect(readdir(outDir).then((files) => files.sort())).resolves.toEqual([
      "mcp.json",
